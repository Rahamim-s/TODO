...
export default {
  ...
  methods: {
    async refreshPosts () {
      this.loading = true
      this.posts = await taskDataService.getPosts()
      this.loading = false
    },
    ...
    async savePost () {
      if (this.model.id) {
        await taskDataService.updatePost(this.model.id, this.model)
      } else {
        await taskDataService.createPost(this.model)
      }
    ...
    async deletePost (id) {
      if (confirm('Are you sure you want to delete this post?')) {
        ...
        await taskDataService.deletePost(id)
        await this.refreshPosts()
      }
    }
  }
